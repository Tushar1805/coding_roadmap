//! Unique Paths in a Grid

// You are given a matrix grid of n x  m size consisting of values 0 and 1.
// A value of 1 means that you can enter that cell and 0 implies that entry to that cell is not allowed.

// You start at the upper-left corner of the grid (1, 1) and
// you have to reach the bottom-right corner (n, m) such that you
// can only move in the right or down direction from every cell.

// Your task is to calculate the total number of ways of reaching the target modulo (109+7).
// Note: The first (1, 1) and last cell (n, m) of the grid can also be 0

// Example 1:

// Input:
// n = 3, m = 3
// grid[][] = {{1, 1, 1};
//             {1, 0, 1};
//             {1, 1, 1}}
// Output:
// 2
// Explanation:
// 1 1 1
// 1 0 1
// 1 1 1
// This is one possible path.
// 1 1 1
// 1 0 1
// 1 1 1
// This is another possible path.
// Example 2:

// Input:
// n = 1, m = 3
// grid = {{1, 0, 1}}
// Output :
// 0
// Explanation:
// There is no possible path to reach the end.

// Your Task:
// You don't need to read input or print anything.
// Your task is to complete the function uniquePaths()
// which takes 2 integers n, and m, and a matrix of size n*m as input
// and returns the number of unique paths from cell (1,1) to (n,m) modulo (109+7)

// Expected Time Complexity: O(n*m)
// Expected Auxiliary Space: O(n*m)

// Constraints:
// 1 ≤ n*m ≤ 106

#include<bits/stdc++.h>
using namespace std;

int mod = 1000000007;

//! Using Recursion
int find(int i, int j, int n, int m, vector<vector<int>>& grid, vector<vector<int>>&dp) {
	if (i < n && j < m) {
		if (i == n - 1 && j == m - 1) return 1;

		if (grid[i][j] == 0 ) return 0;

		// if(i>=n || j>=m || grid[i][j] == 0 ) return 0;

		if (dp[i][j] != -1) return (dp[i][j]) % mod;

		int right = find(i, j + 1, n, m, grid, dp);
		int down = find(i + 1, j, n, m, grid, dp);

		return dp[i][j] = ( right % mod + down % mod) % mod;
	} else {
		return 0;
	}

}
// int uniquePaths(int n, int m, vector<vector<int>> &grid) {
// 	// code here
// 	vector<vector<int>> dp(n, vector<int>(m, -1));
// 	if (grid[0][0] == 0 || grid[n - 1][m - 1] == 0)
// 		return 0;
// 	// int ans =  (find(1, 0, n, m, grid, dp) + find(0, 1, n, m, grid, dp)) % mod;
// 	return (find(0, 0, n, m, grid, dp)) % mod;

// }

//! Using DP without recursion looks easy for understanding
int uniquePaths(int n, int m, vector<vector<int>> &grid) {
	// code here
	vector<vector<int>> dp(n, vector<int>(m, -1));
	if (grid[0][0] == 0 || grid[n - 1][m - 1] == 0)
		return 0;
	for (int i = 0; i < n; i++) {
		int up = 0, left = 0;
		for (int j = 0; j < m; j++) {
			if (grid[i][j] == 0) dp[i][j] = 0;
			else if (i == 0 && j == 0) dp[i][j] = 1;
			else {
				if (i > 0) up = dp[i - 1][j];
				if (j > 0) left = dp[i][j - 1];

				dp[i][j] = (up % mod + left % mod) % mod;
			}
		}
	}
	return dp[n - 1][m - 1] % mod;

}


int main(int argc, char const *argv[])
{
	int n, m;
	cout << "Enter the number of rows and columns: ";
	cin >> n >> m;

	vector<vector<int>> grid;
	cout << "Enter " << n*m << " elements of the grid: ";
	for (int i = 0; i < n; i++) {
		vector<int> temp;
		for (int j = 0; j < m; j++) {
			int data;
			cin >> data;
			temp.push_back(data);
		}
		grid.push_back(temp);
	}

	int ans = uniquePaths(n, m, grid);
	cout << "Number of unique paths in the grid are: " << ans;
	return 0;
}

