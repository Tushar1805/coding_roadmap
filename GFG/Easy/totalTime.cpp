...
// You are given an array arr of size n, containing the values in between 1 to n & time of size n, 
// containing time in sec, you are asked to determine the total time taken in order to pick 
// all the array elements from left to right but there is a condition, If from left, 
// previous elements are different it takes 1 Sec to pick & if element got repeated then it will take time[arr[i]].
...
// Your Task:
// You don't need to read input or print anything. Your task is to complete the function totalTime() 
// which takes three variables n, arr, time, as explained in the problem statement, 
// and returns the total time taken to pick all the elements.
...
int main(int argc, char const *argv[])
{
	...
	cout<<"\nEnter the time for every element in the array: ";
	for (int i = 0; i < n; ++i)
	{
		int value;
		cin>>value;
